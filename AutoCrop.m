...
imshow(rgbImage);
title('İşlemi tamamlamak için çift tıklayın', 'FontSize', fontSize);
 
promptMessage = sprintf('seçmek istediğiniz bölgeyi imleç ile belirleyiniz.\nİşlemi tamamlamak için çift tıklayın.');
titleBarCaption = 'devam etmek istiyor musunuz?';
button = questdlg(promptMessage, titleBarCaption, 'seç', 'iptal', 'seç');
if strcmpi(button, 'iptal')
	return;
end
hBox = imrect;
